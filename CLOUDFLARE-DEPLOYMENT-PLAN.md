...
账号审批升级已完成兼容过渡、独立 D1 迁移、完整代码推送和正式站验收。各阶段的实际结果见[cloud/DEPLOYMENT.md](cloud/DEPLOYMENT.md)。
...
GitHub LeonZ03/Whisper · main
  → Cloudflare Workers Builds（完整升级提交已自动构建并发布）
...
账号升级已按过渡兼容代码、单独审核迁移、完整代码推送、线上验收的顺序完成。过渡提交的自动构建失败，随后使用仅代码手动部署；完整升级提交的自动构建成功。回滚代码不得重新启用旧直接注册，也不得回滚或重置数据库。
